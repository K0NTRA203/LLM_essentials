...
class ChatGPT:
    ...
    def __init__(self, headless: bool = True, browser="firefox", timeout=60):
        ...
        self.headless = False
        # self.play = async_playwright().start()
        self.is_open= False
        self.tokenz = False
    ...
    async def initialize(self):
        ...
        if self.is_open == False:
            ...
            try:
                playbrowser = getattr(self.play, self.browser)
            except Exception:
                print(f"Browser {self.browser} is invalid, falling back on firefox")
                playbrowser = await self.play.firefox
            if self.browser is not None and self.is_open == False:
                ...
                if len(self.browser.pages) > 0:
                    print('*************************PAGE EXIIIISSSSSSSTS*******************')
                    self.page = self.browser.pages[0]
                    if self.is_open == False: 
                        await self._start_browser()
                        self.is_open =True
                else:
                    print('*************************OPENING NEW PAGEEEEE*******************')
    ...
    async def ask_stream(self, prompt: str, token=''):
        if self.session is None and self.tokenz == False:
        ...
        if "accessToken" not in self.session:
            yield (
                "ERR.\n"
                "* Refresh session.\n"
                "* If error persists run $ chatgpt install"
            )
            return
    ...
    async def ask(self, message: str,token = '') -> str:
        """
        ...
        if token == '':
            response = []
            async for res in self.ask_stream(message):
                response.append(res)
        else:
            response = list(self.ask_stream(message,token))
